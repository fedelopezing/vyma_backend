...
- **Architecture Convention Alignment (MANDATORY):** During task generation, the Tech-Lead must ensure that all generated tasks in `docs/tasks/` strictly align with the naming conventions, directory structure, and technical rules detailed in the project [CONSTITUTION.md](file:///c:/Users/fedel/NestJs/vyma_backend/CONSTITUTION.md). Ensure that tasks map specifically to the folders listed in Section 2, use kebab-case and the correct suffixes from Section 3, and comply with the **Tech Lead (Creador de RFC)** role described in **Section 16**.
