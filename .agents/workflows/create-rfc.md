...
- **Architecture Convention Alignment (MANDATORY):** The Architect must strictly consult and follow the project [CONSTITUTION.md](file:///c:/Users/fedel/NestJs/vyma_backend/CONSTITUTION.md) when designing database schemas, naming files, defining directory layouts, and structuring endpoints. They must plan all required files under the correct folder hierarchy of Section 2 and explicitly define DTOs/contracts (including Swagger and validation decorators) before creating task tickets.
