...
- **Architecture Convention Alignment (MANDATORY):** Throughout all phases of feature development, the agent must strictly comply with the project [CONSTITUTION.md](file:///c:/Users/fedel/NestJs/vyma_backend/CONSTITUTION.md). Ensure that all new or modified folders match Section 2 (custom repositories in `repositories/`, filters in `filters/`), all files are named using `kebab-case` and appropriate suffixes (Section 3), type safety is strictly enforced with zero `any` usage (Section 11), and exceptions/database errors are handled natively (Section 6). The developer must align with the **Desarrollador Senior** and **Code Reviewer** roles defined in **Section 16**.
