...
- **Architecture Convention Alignment (MANDATORY):** You must strictly adhere to the guidelines, naming conventions, folder structures, and role instructions defined in [convencion-arquitectura.md](file:///c:/Users/fedel/NestJs/vyma_backend/docs/convencion-arquitectura.md). Act strictly according to the **Creador de RFC (Tech Lead)** role described in **Section 5** of that document: map out exact file paths (following Section 2) and specify DTO contracts in RFCs before execution.
