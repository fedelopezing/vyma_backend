...
describe('WhatsappConnectionService', () => {
  ...
  describe('onModuleInit', () => {
    it('should log and return if WHATSAPP_ENABLED is not true', async () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('WhatsApp connection is disabled (WHATSAPP_ENABLED is not set to true)');
    });
  ...
  describe('onModuleDestroy', () => {
    ...
    it('should catch and log error if client.destroy fails', async () => {
      ...
      expect(loggerWarnSpy).toHaveBeenCalledWith(`Error destroying WhatsApp client: ${error}`);
    });
  });
  ...
  describe('initializeClient (private)', () => {
    ...
    it('should catch error on initialize and schedule reconnect', async () => {
      ...
      const scheduleSpy = jest.spyOn(service as any, 'scheduleReconnect').mockImplementation(() => {});
  ...
  describe('scheduleReconnect (private)', () => {
    ...
    it('should execute reconnect callback after delay', async () => {
      ...
      const buildClientSpy = jest.spyOn(service as any, 'buildClient').mockResolvedValue(undefined);
      const initializeClientSpy = jest.spyOn(service as any, 'initializeClient').mockResolvedValue(undefined);
