...
*   **Usuario (`User`) y Membresía (`UserCompany`):** En el esquema multi-tenant, los roles están definidos exclusivamente a nivel de la membresía (`UserCompany`). Así, un usuario posee un rol específico por cada empresa a la que pertenece (por ejemplo, `admin` en la Empresa A y `professional` en la Empresa B). Se eliminó la relación directa `role` en la entidad `User`.
